...
import { Calendar, Clock, Scissors, CheckCircle, Lock, User, BookmarkCheck } from 'lucide-react';
...
export default function BarberCastel() {
  ...
  // PERSISTENCIA: Cargar citas de LocalStorage al iniciar
  const [appointments, setAppointments] = useState(() => {
    const saved = localStorage.getItem('barber_appointments');
    return saved ? JSON.parse(saved) : [];
  });

  // PERSISTENCIA: Guardar citas automáticamente
  useEffect(() => {
  ...
  const addAppointment = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    alert(`¡Cita agendada con éxito para ${newApp.name}! El dueño ya puede verla en su agenda.`);
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans pb-24">
      ...
      {/* HEADER CON LOGOTIPO VECTORIAL BC */}
      <header className="p-6 border-b border-amber-900/30 bg-black sticky top-0 z-50 flex justify-between items-center shadow-2xl">
        <div className="flex items-center gap-3">
          {/* LOGOTIPO BC: TIJERA Y NAVAJA CRUZADAS */}
        ...
        <button 
          onClick={view === 'client' ? handleAdminAccess : () => setView('client')}
          className={`text-[10px] font-black px-4 py-2 rounded-lg border transition-all ${
            view === 'client' 
            ? 'bg-neutral-900 text-neutral-400 border-neutral-800 hover:text-amber-500' 
      ...
      <main className="p-6 max-w-md mx-auto">
        {view === 'client' ? (
          <section className="animate-in zoom-in-95 duration-500">
            ...
            <form onSubmit={addAppointment} className="space-y-4 bg-neutral-900/40 p-6 rounded-3xl border border-neutral-800 shadow-2xl backdrop-blur-md">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-amber-700 uppercase ml-1">Tu Nombre</label>
                <input name="clientName" required className="w-full bg-neutral-950/50 border border-neutral-800 p-4 rounded-xl focus:border-amber-500 outline-none text-white transition-all" placeholder="Ej. Juan Pérez" />
              </div>
              
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-amber-700 uppercase ml-1">Servicio</label>
                <select name="service" className="w-full bg-neutral-950/50 border border-neutral-800 p-4 rounded-xl focus:border-amber-500 outline-none text-white appearance-none">
                  ...
                  <option>Degradado Premium</option>
                </select>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-amber-700 uppercase ml-1">Hora</label>
          ...
          <section className="animate-in slide-in-from-bottom duration-500 text-white">
            <div className="flex justify-between items-center mb-8 bg-neutral-900/50 p-4 rounded-2xl border border-neutral-800 shadow-md">
              <div>
                <h2 className="text-xl font-black text-amber-500 italic uppercase">Agenda BC</h2>
            ...
            <div className="space-y-4">
              {appointments.length === 0 ? (
                <div className="text-center py-20 border-2 border-dashed border-neutral-900 rounded-3xl">
                  <p className="text-neutral-600 font-bold uppercase text-xs tracking-widest italic">No hay registros hoy</p>
                </div>
              ) : (
                appointments.map((app) => (
                  <div key={app.id} className="bg-neutral-900 border-l-4 border-amber-600 p-5 rounded-xl flex justify-between items-center group hover:border-amber-500/40 transition-all shadow-md">
                    ...
                    <button 
                      ...
                      className="p-2 text-neutral-800 hover:text-green-500 transition-colors"
